...
└── config/
    ...
    ├── ssh/
    │   └── config               # SSH client: TERM fallback (xterm-256color) for remote servers
    ├── git/
    ...
    │   ├── skills/              # Global skills (matt-review from mattpocock/skills)
    │   └── commands/            # Global commands (/matt-review)
    └── brew/
...
make install-zsh-edit-select       # Install or re-sync the zsh-edit-select plugin + macOS agent at pinned, digest-verified revisions (under ~/.local/share/zsh/plugins/)
...
make copy-ssh                     # config/ssh/config → ~/.ssh/config
...
make copy-opencode-skills          # → ~/.config/opencode/skills/
make copy-opencode-commands        # → ~/.config/opencode/commands/
...
- **SSH TERM fallback:** Ghostty sets `TERM=xterm-ghostty` which is absent from most remote servers. `config/ssh/config` uses `Host * SetEnv TERM=xterm-256color` (OpenSSH ≥8.7) to avoid ncurses "Error opening terminal" errors over SSH.
...
- **Dual-terminal zsh gate:** The `.zshrc` blocks that load powerlevel10k, autosuggestions, zsh-edit-select, syntax-highlighting, fuzzy completion, and history-substring-search are gated on `$TERM_PROGRAM` matching either `WezTerm` or `ghostty`. Other terminals get a minimal shell. The `bindkey "^U" backward-kill-line` line is terminal-agnostic (unconditional) and pairs with both WezTerm's and Ghostty's Cmd+Backspace mapping.
- **zsh-edit-select is make-managed and release-pinned, not Homebrew:** no formula exists and the repo deliberately has no plugin manager. `make install-zsh-edit-select` clones to `~/.local/share/zsh/plugins/zsh-edit-select`, checks out `ZSH_EDIT_SELECT_REF` (full upstream commit SHA, re-verified with `git rev-parse`; aborts on mismatch or unfetchable pin), and pre-seeds the macOS clipboard agent from the pinned `ZSH_EDIT_SELECT_RELEASE` with fail-closed SHA-256 verification — it aborts when `SHA256SUMS.txt` is unavailable or empty, when the asset has no matching entry, when `shasum` is missing, when the entry disagrees with the reviewed `ZSH_EDIT_SELECT_AGENT_SHA256`, or on any download-vs-digest mismatch. Pre-seeding matters: the plugin's own runtime fetcher (`assets/fetch-agents.zsh`) is deliberately best-effort upstream — it installs UNVERIFIED when the sums file, matching entry, or SHA-256 tool is missing, and it downloads from `releases/latest` — but its `[[ ! -x ]]` guard in `plugin.zsh` never triggers once make has installed the agent. Routine runs never accept unreviewed upstream code: updates are an explicit, reviewed change bumping `REF`/`RELEASE`/`AGENT_SHA256` together in the Makefile. `.zshrc` sources it existence-guarded inside the gated block, before zsh-syntax-highlighting (ZLE widget ordering: widget-defining plugins must load before it). WezTerm/Ghostty configs carry the plugin's macOS CSI-u keybindings (Cmd+A/C/V/X/Z, Cmd+Shift+arrows); without the plugin installed those keys are dead. In WezTerm, Cmd+Shift+Left/Right replaced MoveTabRelative with select-to-line-start/end (tab drag still works). WezTerm mouse_bindings deliberately keep copy-on-select — the plugin's mouse integration is experimental on macOS and needs `edit-select setup-ax` + Accessibility permission; adopt the plugin's recommended mouse config only if wanted.
...
- **Third-party skills:** The `matt-review` skill under `config/opencode/skills/` is sourced from [mattpocock/skills](https://github.com/mattpocock/skills) v1.2.0 (MIT). It is vendored as-is with only the frontmatter `name` changed. Update by re-fetching from upstream.
