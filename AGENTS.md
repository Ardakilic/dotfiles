...
- **Command-line selection:** `.zshrc` inlines an adaptation of zsh-shift-select (MIT, Jakub Jirutka). Shift+Arrows / Shift+Home/End (xterm sequences `\e[1;2D` etc.) select command-line text via a dedicated `shift-select` keymap; Backspace/Delete kill the region, Option+W (`\ew`) copies region to kill ring + `pbcopy`, any other key deselects and replays. Works in both terminals: WezTerm passes the sequences through via explicit `DisableDefaultAssignment` bindings; Ghostty's performable `adjust_selection` defaults fall through to the shell when no terminal selection exists (explicit performable keybinds are documented in `config.ghostty`).
- **Mouse selection:** Ghostty `copy-on-select = clipboard` (drag copies on release) and `mouse-shift-capture = never` (Shift+drag extends a selection even in mouse-reporting apps like vim/tmux; cannot be overridden by XTSHIFTESCAPE). Ghostty's native Shift+Arrow `adjust_selection` defaults adjust an existing mouse selection and typing clears it (`selection-clear-on-typing`, default true). WezTerm's Shift+click extend-selection and Shift+drag are built-in defaults. Note: terminal mouse selections are NOT synchronized with ZLE — deleting a mouse-selected chunk of command-line text via Backspace does not edit the zsh buffer; that requires a plugin like [zsh-edit-select](https://github.com/Michael-Matta1/zsh-edit-select) (see README TODO).
