...
- **Mouse selection:** Ghostty `copy-on-select = clipboard` (drag copies on release) and `mouse-shift-capture = never` (Shift+drag extends a selection even in mouse-reporting apps like vim/tmux; cannot be overridden by XTSHIFTESCAPE). Ghostty's native Shift+Arrow `adjust_selection` defaults adjust an existing mouse selection and typing clears it (`selection-clear-on-typing`, default true). WezTerm gets an explicit `ExtendSelectionToMouseCursor` Shift+Left-Up binding. Deleting mouse-selected command-line text = select, then Backspace.
