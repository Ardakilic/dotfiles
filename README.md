...
make install-zsh-edit-select        # Install or re-sync the zsh-edit-select plugin at its pinned commit
```
...
* **zsh-edit-select** is the only zsh plugin not installed via Homebrew: `make install-zsh-edit-select` clones it to `~/.local/share/zsh/plugins/zsh-edit-select` **pinned to a reviewed upstream commit** (`ZSH_EDIT_SELECT_REF` in the Makefile). Every run checks out exactly that commit and verifies the checked-out SHA, aborting on mismatch; routine runs never pull the mutable upstream branch — updating means reviewing the upstream diff and bumping `ZSH_EDIT_SELECT_REF` as an explicit change to this repo. The plugin's agent binaries are fetched by the plugin itself and verified against the upstream release's `SHA256SUMS.txt` (the fetch aborts if the sums file, matching entry, or a SHA-256 tool is unavailable). The WezTerm and Ghostty configs ship the plugin's macOS keybindings (Cmd+A/C/V/X/Z, Cmd+Shift+arrows); those keys are inert until the plugin is installed. In WezTerm, Cmd+Shift+Left/Right now select to line start/end (they previously moved the current tab; drag still moves tabs). Mouse integration is optional and experimental on macOS — run `edit-select setup-ax` and grant Accessibility permission if you want it; keyboard selection works out of the box. Customize everything with `edit-select config`.
