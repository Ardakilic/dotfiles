...
make copy-zsh-plugins              # config/zsh/zsh-edit-select/ → ~/.config/zsh/zsh-edit-select/
...
- **Command-line selection:** `.zshrc` sources the vendored [zsh-edit-select](https://github.com/Michael-Matta1/zsh-edit-select) plugin (v0.7.00, MIT, © 2025 Michael Matta) from `~/.config/zsh/zsh-edit-select/` (copied by `make copy-zsh-plugins`; source of truth `config/zsh/zsh-edit-select/`, update by re-fetching upstream — Linux impls and benchmark/test/auto-install assets are trimmed). Text-editor-style editing: Shift+Arrows select; Cmd+A/C/X/V select-all/copy/cut/paste-over; typing replaces the selection; Backspace/Delete deletes it; Cmd+Z/Cmd+Shift+Z undo/redo; mouse selections integrate (type/paste/cut/delete a mouse-selected chunk). The macOS Cmd keys arrive via CSI-u sequences remapped in both terminal configs (`cmd+a=csi:97;9u` etc. in `config.ghostty`; `SendString '\x1b[97;9u'` etc. in `.wezterm.lua`). First shell start downloads/compiles a small clipboard agent (pbcopy/pbpaste fallback); `edit-select setup-ax` + Accessibility permission enable the full macOS mouse path.
- **Mouse selection:** Ghostty `copy-on-select = clipboard` (drag copies on release) and `mouse-shift-capture = never` (Shift+drag extends a selection even in mouse-reporting apps like vim/tmux; cannot be overridden by XTSHIFTESCAPE). Ghostty's native Shift+Arrow `adjust_selection` defaults adjust an existing mouse selection and typing clears it (`selection-clear-on-typing`, default true). WezTerm's Shift+click extend-selection and Shift+drag are built-in defaults; a left-click `action_callback` sends the plugin's deselect signal (`\x1b[>62300u`, skipped on the alt screen) so stale selections aren't edited. Note: WezTerm/Ghostty don't expose macOS Accessibility attributes, so the plugin's mouse integration there uses its experimental reactive path; iTerm2/Kitty would be the reliable-AAX terminals.
