...
- [zsh-edit-select](https://github.com/Michael-Matta1/zsh-edit-select) — editor-like command line editing (Shift selection, cut/copy/paste, type-to-replace, undo/redo). Not on Homebrew; installed at a pinned commit via `make install-zsh-edit-select`
...
make install-zsh-edit-select        # Install or re-sync the zsh-edit-select plugin + agent at pinned, digest-verified revisions
```
...
* **zsh-edit-select** is the only zsh plugin not installed via Homebrew: `make install-zsh-edit-select` clones it to `~/.local/share/zsh/plugins/zsh-edit-select` **pinned to one reviewed upstream release** (source commit `ZSH_EDIT_SELECT_REF` + release tag `ZSH_EDIT_SELECT_RELEASE` + reviewed agent digest `ZSH_EDIT_SELECT_AGENT_SHA256`, all in the Makefile). Every run checks out exactly that commit (SHA re-verified, abort on mismatch) and pre-seeds the macOS clipboard agent from the pinned release with fail-closed SHA-256 verification: a missing `SHA256SUMS.txt`, missing matching entry, missing `shasum`, disagreement between the entry and the reviewed digest, or any digest mismatch aborts the install. Pre-seeding keeps runtime tied to the pin — the plugin's own first-load fetcher is best-effort upstream (it installs unverified when sums/entry/tool are missing and pulls from `releases/latest`), but it only runs when the agent binary is absent. Updating means reviewing the upstream diff and bumping the three pin variables together. The WezTerm and Ghostty configs ship the plugin's macOS keybindings (Cmd+A/C/V/X/Z, Cmd+Shift+arrows); those keys are inert until the plugin is installed. In WezTerm, Cmd+Shift+Left/Right now select to line start/end (they previously moved the current tab; drag still moves tabs). Mouse integration is optional and experimental on macOS — run `edit-select setup-ax` and grant Accessibility permission if you want it; keyboard selection works out of the box. Customize everything with `edit-select config`.
