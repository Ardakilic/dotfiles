...
* **Mouse selection (both terminals).** Drag-select copies on release (`copy-on-select` in Ghostty, `CompleteSelection` in WezTerm). In Ghostty, Shift+drag extends a selection even in mouse-reporting apps (`mouse-shift-capture = never`), Shift+Arrows adjust an existing mouse selection, and typing/deleting clears it. In WezTerm, Shift+click extends the selection to the mouse cursor. To delete a chunk of command-line text with the mouse in WezTerm/Ghostty: select it, then press `Backspace` (see the `zsh-edit-select` plugin in the TODO notes below for full mouse-delete integration).
...
- [x] Add Shift+Arrow command-line selection + mouse selection polish (both terminals)
