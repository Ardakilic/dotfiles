...
- **Shell:** zsh (powerlevel10k theme, zinit plugin manager)
...
- **Dual-terminal zsh gate:** The `.zshrc` blocks that load zinit, zsh-edit-select, powerlevel10k, autosuggestions, syntax-highlighting, fuzzy completion, and history-substring-search are gated on `$TERM_PROGRAM` matching either `WezTerm` or `ghostty`. Other terminals get a minimal shell. The `bindkey "^U" backward-kill-line` line is terminal-agnostic (unconditional) and pairs with both WezTerm's and Ghostty's Cmd+Backspace mapping.
- **zinit is Homebrew-managed:** `brew "zinit"` in config/brew/Brewfile, installed via `make install-deps`. `.zshrc` sources the brew path existence-guarded inside the `$TERM_PROGRAM` gated block; plugins are declared as `zinit` statements there, cloned under zinit's data dir on first load.
- **zsh-edit-select is zinit-managed:** declared in `.zshrc`'s `$TERM_PROGRAM` gated block right after the zinit bootstrap (`zinit depth"1" light-mode for Michael-Matta1/zsh-edit-select`, guarded by `(( $+functions[zinit] ))`). It must load before zsh-syntax-highlighting (it defines ZLE widgets). zinit clones it under `~/.local/share/zinit/plugins/`; its first load downloads the macOS clipboard agent binary via the plugin's own fetcher. WezTerm/Ghostty keybinding facts are documented in the CSI-u bullet above.
- **Terminal CSI-u keybindings for editor-like command line editing:** The WezTerm and Ghostty configs ship the plugin's macOS CSI-u keybindings (Cmd+A/C/V/X/Z, Cmd+Shift+arrows); those keys are dead unless the zsh-edit-select plugin is loaded. In WezTerm, Cmd+Shift+Left/Right replaced MoveTabRelative with select-to-line-start/end (tab drag still works). WezTerm mouse_bindings deliberately keep copy-on-select — the plugin's mouse integration is experimental on macOS and needs `edit-select setup-ax` + Accessibility permission; adopt the plugin's recommended mouse config only if wanted.
