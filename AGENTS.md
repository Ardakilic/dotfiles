...
make install-zsh-edit-select       # Install or re-sync the zsh-edit-select plugin at its pinned commit (ZSH_EDIT_SELECT_REF) under ~/.local/share/zsh/plugins/
...
- **zsh-edit-select is make-managed and commit-pinned, not Homebrew:** no formula exists and the repo deliberately has no plugin manager. `make install-zsh-edit-select` clones to `~/.local/share/zsh/plugins/zsh-edit-select` and checks out `ZSH_EDIT_SELECT_REF` — a full upstream commit SHA re-verified with `git rev-parse` after checkout (the target aborts on mismatch, or if the pinned commit cannot be fetched). Routine runs never accept unreviewed upstream commits: updates are an explicit, reviewed change bumping `ZSH_EDIT_SELECT_REF` in the Makefile. The plugin's runtime agent downloads are verified by the plugin itself against the upstream release's `SHA256SUMS.txt` (aborts when the sums file, matching entry, or a SHA-256 tool is missing) — that logic is frozen by the commit pin. `.zshrc` sources it existence-guarded inside the gated block, before zsh-syntax-highlighting (ZLE widget ordering: widget-defining plugins must load before it). WezTerm/Ghostty configs carry the plugin's macOS CSI-u keybindings (Cmd+A/C/V/X/Z, Cmd+Shift+arrows); without the plugin installed those keys are dead. In WezTerm, Cmd+Shift+Left/Right replaced MoveTabRelative with select-to-line-start/end (tab drag still works). WezTerm mouse_bindings deliberately keep copy-on-select — the plugin's mouse integration is experimental on macOS and needs `edit-select setup-ax` + Accessibility permission; adopt the plugin's recommended mouse config only if wanted.
